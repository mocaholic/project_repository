	...
	*/
	...
	alert:function(msg, title, callback, obj) {
		...
		if (title != "" && title != undefined) {
			...
			aMsg.push('<div class="header"><p>KBNC</p></div>');
		}
	...
	errorAlert:function(code, msg, callback) {
		...
		aMsg.push('<div class="layer_popup_tit"><div class="tit22"><strong>KBNC</strong></div></div>');
	 ...
	 * @param async : 동기화 여부
	 */
	Ajax2JsonHtml:function(data, url, callback, async, dataType, loading) {
		if (KBNC.isAjax2ObjLoading) return;
		KBNC.isAjax2ObjLoading = true;
		if (loading || loading == "undefined" || loading == "" || loading == null || loading == undefined) {
			KBNC.Loading(true);
	...
	fnShippingAddrOpen: function(fnCallback) {
		...
		window.open(uurl, "_shippingAddr");
