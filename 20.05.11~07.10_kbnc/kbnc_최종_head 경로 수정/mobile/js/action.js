...
ACTIONUI.datePicker = {
    ...
    fn: function(target){
        var _target = $('.chk-calendar .date-input input');
        $(_target).datepicker({
...
ACTIONUI.gnbSwipe = function () {
	...
	function swipeController(gnbSwiper) {
		...
		$gnbItems.on('click', function () {
			$gnbItems.parent('li').removeClass('active');
...
ACTIONUI.sideMenu = function () {
	...
	SideMenu.prototype.addEvent = function () {
		...
		$('#side-menu .category-list li').each(function(idx){
			$(this).on('click', function(){
				if(!$(this).hasClass('on')){
					$('#side-menu .category-list li').removeClass('on');
					$(this).addClass('on');
...
ACTIONUI.moreViewList = {
	...
	fn: function(){
		var $moreViewBox    = $('.box-more-view'),
			$cateList       = $moreViewBox.find('.category-list li'),
			$moreBtn        = $moreViewBox.find('.btn-more-view'),
			$viewAreaH      = $(".box-more-view").outerHeight(),
			$viewAreaT      = $(document).scrollTop(),
			$viewAreaS      = $viewAreaT - $viewAreaH + 1010;

		if($cateList.length <= 6){
			//카테고리 리스트 6개이하일때 더보기 버튼 숨김
			$cateList.parent('ul').next('.btn-area').hide();
		}
...
ACTIONUI.productKeep = {
    ...
    fn: {
        button: function(){
            $('.btn-keep').off('click').on('click', function(e){
				e.preventDefault();
				$('#loading').hide();  
...
//주문결제 아코디언
ACTIONUI.payAccordion = {
    onloadFlag: true,
    init: function(){
        var acTitle = $('.pay-cont .ord-tit .tit a');
...
//약관동의 아코디언
ACTIONUI.agreeAccordion = {
    onloadFlag: true,
    init: function(){
        var acTitle = $('.agree-head .btn-arw');

        acTitle.click(function(){  
            if(!$(this).parent().hasClass('on')){
                $(this).parent().addClass('on');
                $(this).parent('.agree-head').next('.agree-txt').show(); 
            } else{
				$(this).parent().removeClass('on');
                $(this).parent('.agree-head').next('.agree-txt').hide(); 
            }
            return false;
         });
    }
...
ACTIONUI.optionToggle = {
    ...
    fn: function(target){
        ...
        $(_target).on('click', function(e){
            ...
            if(!$(this).hasClass('on')){
				$(this).addClass('on');
				$(this).parent().parent().parent().next('.quan-inp-wrap').show();
            }else{
				$(this).removeClass('on');
				$(this).parent().parent().parent().next('.quan-inp-wrap').hide();
            };
        });
    }
}
